function results_export_strategies    
  ...
  %  assert(size(class_distr, 1) == traj.count);
    ...
    for i = 1:traj.count
        data(i, 1:7) = [traj.items(i).group, traj.items(i).id, traj.items(i).session, traj.items(i).trial, partitions(i), base_config.FEATURE_LATENCY.compute(traj.items(i)), base_config.FEATURE_AVERAGE_SPEED.compute(traj.items(i))]; 
        if partitions(i) > 0
