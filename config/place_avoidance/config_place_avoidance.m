classdef config_place_avoidance < base_config
    ...
    properties(Constant)        
        ...
        FEATURE_ANGULAR_DISTANCE_SHOCK_SIGNED = trajectory_feature('D_ang3', 'Ang. distance shock signed', 'trajectory_mean_angle_signed', 1, ...
            {'CENTRE_X', 'CENTRE_Y', 'SHOCK_AREA_ANGLE'}, 'Mirror', 1 );        
        ...
        FEATURE_VAR_ANG_VELOCITY_ARENA = trajectory_feature('Var_Vang_A', 'Variance angular velocity (arena)', 'trajectory_angular_velocity', ...
            2, {'CENTRE_X', 'CENTRE_Y'});                                     
        FEATURE_AVG_ANG_VELOCITY_ARENA_0 = trajectory_feature('V0_ang_A', 'Average angular velocity at start (arena)', 'trajectory_angular_velocity', ...
            1, {'CENTRE_X', 'CENTRE_Y'}, 'TLength', 1);
        FEATURE_MAX_ANG_VELOCITY_ARENA_0 = trajectory_feature('V0max_ang_A', 'Max average angular velocity at start (arena)', 'trajectory_angular_velocity', ...
            3, {'CENTRE_X', 'CENTRE_Y'}, 'TLength', 1);
        FEATURE_LOG_RADIUS_0 = trajectory_feature('log_R12_0', 'Log radius at start', 'trajectory_radius', 1, ...
            {'CENTRE_X', 'CENTRE_Y', 'ARENA_R'}, ...
            'TLength', 1, ...
            'TransformationFunc', config_place_avoidance.FUNCTION_LOG_NEG, ...
            'AveragingFunc', config_place_avoidance.FUNCTION_MEAN );
    ...
    methods        
        ...
        function draw_arena(inst, traj, data_repr)
            ...
            if data_repr.hash_value ~= inst.DATA_REPRESENTATION_ARENA_COORD.hash_value
                % draw shock area            
                ang = inst.property('SHOCK_AREA_ANGLE', []);
                ra = inst.property('ARENA_R');
                x0 = inst.property('CENTRE_X');
                y0 = inst.property('CENTRE_Y');

                if ~isempty(ang)
                    % draw a 60 deg shock area
                    angi = ang(traj.trial) - pi/6;
                    angf = ang(traj.trial) + pi/6;
                    harc = plot_arc(angi, angf, x0, y0, ra*.99);
                    set(harc,'edgecolor', 'r', 'linewidth', .1, 'facecolor', 'r');
                    alpha(harc, 0.3);
                end
            end
        end
