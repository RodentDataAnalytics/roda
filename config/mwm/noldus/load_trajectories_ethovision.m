function [ traj, cal_data ] = load_trajectories_ethovision(config, path, setn, path_calibration, varargin)
...
%LOAD_TRAJECTORIES Loads a set of trajectories from a given folder          
    ...
    if ~isempty(path_calibration)
        cal_data = load_calibration_data(path_calibration, path);
    end
    ...
    for j = 1:length(files)           
        ...
        if day > 1
            itrial = sum(config.TRIALS_PER_SESSION(1:day - 1)) + 1;
            if trial < itrial                   
        ...
        % chop points at the end on top of the platform            
         ...
         for k = 0:(size(pts, 1) - 1)
             if sqrt((pts(npts - k, 2) - config.property('PLATFORM_X'))^2 + (pts(npts - k, 3) - config.property('PLATFORM_Y'))^2) > config.property('PLATFORM_R');
                 break;
             end
             cuti = npts - k - 1;
         end
